...
if (isset($_GET['payment_id'], $_GET['work_id'], $_GET['amount'])) {
    ...
    // echo "Work ID: " . htmlspecialchars($work_id) . "<br>";
            ...
            <div class="h-full">
                <div class="relative px-4 sm:px-6 lg:px-8 pb-8 max-w-lg mx-auto" x-data="{ card: true }">
                    <div class="bg-white px-8 pb-6 rounded-b">
                        ...
                        <form action="pay_online_period.php" method="post" enctype="multipart/form-data">
                            ...
                            <div x-show="card">
                                ...
                                <div class="mt-6">
                                    <div class="mb-4">
                                        <button name="pay_online_banking" type="submit" class="font-medium text-sm inline-flex items-center justify-center px-3 py-2 border border-transparent rounded leading-5 shadow-sm transition duration-150 ease-in-out w-full bg-indigo-500 hover:bg-indigo-600 text-white focus:outline-none focus-visible:ring-2">Pay <?php echo htmlspecialchars($amount); ?> THB</button>
                                    </div>
                            ...
                            <div class="mb-4">
                                <form name="checkoutForm" method="POST" action="checkout.php">
                                    <div class="w-full font-medium text-sm inline-flex items-center justify-center px-3 py-2 border border-transparent rounded leading-5 shadow-sm transition duration-150 ease-in-out  bg-indigo-500 hover:bg-indigo-600 text-white focus:outline-none focus-visible:ring-2">
                                        <input type="hidden" name="package" value="<?php echo htmlspecialchars($payment_id) ?>">
                                        <input type="text" name="type" value="<?php echo $work_id ?>" hidden>
                                        <script type="text/javascript" src="https://cdn.omise.co/omise.js" data-key="pkey_test_5z82pmcct8a9vkowopg" data-image="https://scontent.fbkk22-8.fna.fbcdn.net/v/t39.30808-6/432995021_930444285426683_3261725009997351016_n.jpg?_nc_cat=110&ccb=1-7&_nc_sid=5f2048&_nc_ohc=Pk_r8HMe0pwAX_dihTP&_nc_ht=scontent.fbkk22-8.fna&oh=00_AfAMK6ZxOShGcVzjH_FcaDBFo_is_1iKsySkq9HXsh_4SA&oe=6609A53B" data-frame-label="Best Contrac" data-button-label="Credit / Debit" data-submit-label="ชำระเงิน" data-location="no" data-amount="<?php echo htmlspecialchars($amount); ?>" data-currency="thb">
                                        </script>
                                    </div>
                                </form>
                            </div>
