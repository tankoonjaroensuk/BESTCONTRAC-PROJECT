...
$sql_con_member = "SELECT * FROM contractor_member";
$result = $conn->query($sql_con_member);

if ($result === false) {
    die("Error: " . $conn->error);
}
$sql_contractor = "SELECT * FROM userprofile WHERE user_type = 'contractor'";
$result2 = $conn->query($sql_contractor);
...
<body>
    ...
    <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
        <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
            <ul class="space-y-2 font-medium">
                ...
                <li>
                    <a href="contractor.php" class="flex items-center p-2 text-white rounded-lg dark:text-white  bg-red-800 group">
    ...
    <div class="max-w-screen-xl mx-auto px-4 md:px-0 mt-20 pt-10">
        <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
            <div class="max-w-lg mt-10">
                ...
                <p class="text-gray-600 mt-2">
                    ข้อมูลส่วนตัวผุ้ว่าจ้างและผู้รับเหมาทั้งหมด
                </p>
            </div>
            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="default-tab" data-tabs-toggle="#default-tab-content" role="tablist">
                <li class="me-2" role="presentation">
                    <button class="inline-block p-4 border-b-2 rounded-t-lg" id="profile-tab" data-tabs-target="#profile" type="button" role="tab" aria-controls="profile" aria-selected="false">ผู้รับเหมา</button>
                </li>
                <li class="me-2" role="presentation">
                    <button class="inline-block p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300" id="dashboard-tab" data-tabs-target="#dashboard" type="button" role="tab" aria-controls="dashboard" aria-selected="false">สมาชิกผู้รับเหมา</button>
                </li>
                ...
                <div class="max-w-screen-xl mx-auto px-4 md:px-0">
                    <div class="items-start  justify-between md:flex">
                        <div class="max-w-lg mt-10">
                            ...
                            <p class="text-gray-600 mt-2">
                                สมาชิกผู้รับเหมาทั้งหมดของผู้รับเหมา
                            </p>
                        </div>
                        ...
                        <table class="w-full table-auto text-sm text-left bg-gray-50">
                            <thead class=" text-gray-600 font-medium border-b">
                                <tr>
                                    ...
                                    <th class="py-3 px-6">contractor</th>
                            ...
                            <tbody class="text-gray-600 divide-y">
                                <?php while ($row2 = $result2->fetch_assoc()) { ?>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($row2['bname']); ?></td>
                                ...
                                <?php while ($row = $result->fetch_assoc()) { ?>
                                    <tr>
                                        <td class="flex items-center gap-x-3 py-3 px-6 whitespace-nowrap">
                                            <img src="image_member/<?php echo htmlspecialchars($row['image_p']); ?>" class="w-10 h-10 rounded-full" />
                                            <div>
                                                <span class="block text-gray-700 text-sm font-medium">ID:
                                                    <?php echo htmlspecialchars($row['member_con']); ?>.<?php echo htmlspecialchars($row['sub_id']); ?></span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($row['sub_n']); ?></td>
                                        <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($row['sub_ln']); ?></td>
                                        <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($row['sub_skill']); ?></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
